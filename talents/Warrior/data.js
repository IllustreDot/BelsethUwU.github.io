...
talent[i] = [0, "Tide of Blood",1,4,2,[getTalentID("Improved Rend"),1]];i++;
...
talent[i] = [0, "In for the kill", 1,4,8,[getTalentID("Unrelenting Assault"),1]]; i++;
...
talent[i] = [1, "Raging Blow", 1,4,4,[getTalentID("Enrage"),1]];i++;
...
talent[i] = [1, "Barbaric Onslaught", 1, 4, 6,[getTalentID("Flurry"),1]];i++;
...
talent[i] = [2, "Blood And Thunder",1,4,2,[getTalentID("Improved Thunder Clap"),1]];i++;
...
talent[i] = [2, "Damage Shield", 2, 1, 10]; i++;
talent[i] = [2, "Sword and Board", 3, 2, 10, [getTalentID("Devastate"),1]]; i++;
talent[i] = [2, "Battering Ram", 1, 3, 10,[getTalentID("Sword and Board"),1]]; i++;
...
//Tide of Blood - Arms	
rank[i] = [
	"20 Rage<br><span style=text-align:left;float:left;>Instant<br>Require Melee Weapon</span><span style=text-align:right;float:right;>Melee range</span><br><br>A heavy strike which grants you 1 stack of Taste for Blood. Can only be used if Rend is active on your target."
	];
i++;	
...
rank[i] = [
	"Your Overpower now grants 10% haste for 4 seconds."
		];
	i++;	
//Sudden Death - Arms	
rank[i] = [
...
//Raging Blow - Fury
rank[i] = [
	"Melee Range<br><span style=text-align:left;float:left;>Instant</span><span style=text-align:right;float:right;>12sec cooldown</span><br>Deal a mighty blow with both weapons. Can only be used if Enrage is active and consumes the Effect."
		];
	i++;	
//Precision - Fury
rank[i]=[
...
i++;
//Barbaric Onslaught - Fury
rank[i] = [
	"<span style=text-align:left;float:left;>50 Rage<br>Channeled</span><br><span style=text-align:right;float:right;>30 sec cooldown</span><br>Instantly attack the target causing 5 damage. In addition, the next 3 successful melee attacks will restore 1% of max health. This effect lasts 8 sec. Damage is based on your attack power."		];
	i++;	
//Intensify Rage - Fury
rank[i] = [
...
i++;	
//Blood and Thunder - Protection	
rank[i] = [
	"Thunder Clap affects 2 nearby enemies with Rend for 12 seconds if they were recently affected by Revenge."
		];
	i++;	
...
rank[i] = [
	"Shield Slam increases your health by 4% for 6 seconds."
		];
	i++;	
//Shockwave - Prtection
rank[i] = [
